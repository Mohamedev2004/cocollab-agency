...
// Define props for the BrandCard component
...
// Sample data for the brands to be displayed.
const brands: BrandProps[] = [
  {
    name: 'Stark Industries',
    description:
      "Leading innovator in defense and clean energy. We partner with top influencers to showcase groundbreaking technologies and futuristic products.",
  ...
  },
  {
    name: 'Rogers Fitness',
    description:
      "We specialize in fitness equipment and lifestyle campaigns. Our mission is to build stronger communities through health and wellness.",
  ...
  {
    name: 'Maximoff Lifestyle',
    description:
      "A creative brand that brings magic into everyday life with fashion, decor, and content partnerships that inspire joy and creativity.",
  ...
  {
    name: 'Romanoff Design Studio',
    description:
      "We craft unforgettable brand identities through design and storytelling. Partnering with influencers to amplify reach and connection.",
...
// The main component that renders a grid of BrandCards.
const App = () => {
  return (
    <div className="flex flex-col items-center bg-[var(--color-cocollab)] justify-center min-h-screen mt-4 rounded-2xl px-6 py-14 font-sans">
      <div className="container mx-auto flex flex-col items-center">
        ...
        <p className="text-gray-200 text-center px-8 max-w-3xl mb-12 sm:text-lg font-light">
          Explore leading brands from different industries. Learn about their vision, 
          stats, and discover collaboration opportunities tailored for influencers like you.
        </p>
...
// BrandCard component displays a single brand's details.
const BrandCard: React.FC<BrandProps> = ({
  name,
  description,
  logo,
  stats,
  ...
  const getProfileRoute = () => {
    if (!user) return route('brand-profile'); // fallback guest
    if (user.role === 'admin') return route('admin.brand-profile');
    if (user.role === 'brand') return route('brand.brand-profile');
    if (user.role === 'influencer') return route('influencer.brand-profile');
    return route('brand-profile');
  };
